...
//US Count from covid-api.com      
   fetch('https://covid-api.com/api/reports?date=2021-03-25&iso=USA')
  .then(response => {
   return response.json();
  })
  .then(data => {
...
//US Count from New York Times
  d3.csv("./nytimes_covid_19_data/nytimes_us_total.csv").then(function(data) {
   ...
   usCaseConfirm.textContent = addCommas(data[429].cases);
   usCaseDeath.textContent = addCommas(data[429].deaths); 
   usCaseRecent.textContent = data[429].date;
  });

//NY Count from New York Times
  d3.csv("./nytimes_covid_19_data/nytimes_daily_reports/03-25-2021.csv").then(function(data) {
   console.log(data);
