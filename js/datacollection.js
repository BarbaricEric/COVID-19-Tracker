...
//US Count from covid-api.com      
   fetch('https://covid-api.com/api/reports?date=2021-03-18&iso=USA')
  .then(response => {
   return response.json();
  })
  .then(data => {
...
//US Count from New York Times
  d3.csv("./nytimes_covid_19_data/nytimes_us_total.csv").then(function(data) {
   ...
   usCaseConfirm.textContent = addCommas(data[422].cases);
   usCaseDeath.textContent = addCommas(data[422].deaths); 
   usCaseRecent.textContent = data[422].date;
  });

//NY Count from New York Times
  d3.csv("./nytimes_covid_19_data/nytimes_daily_reports/03-18-2021.csv").then(function(data) {
   console.log(data);
